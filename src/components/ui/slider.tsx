...
export function Slider({ label, value, onChange, min = 10, max = 50, step = 1, className }: SliderProps) {
  return (
    <div className={cn("flex flex-col gap-2.5 w-full", className)}>
      <div className="flex items-center justify-between">
        <span className="text-[14px] font-semibold text-[#1A1918] font-[Outfit]">{label}</span>
        <span className="text-[14px] font-semibold text-[#3D8A5A] font-[Outfit]">{value}</span>
      </div>
