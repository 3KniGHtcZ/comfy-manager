...
import type { GenerationParams, GeneratedImage } from '~/lib/types'
...
export function useGeneration() {
  ...
  const processNextBatch = useCallback(() => {
    ...
    const currentIndex =
      batchTotalRef.current - batchQueueRef.current.length
    setState((s) => ({
      ...s,
  ...
  useSSE(ssePromptId, {
    ...
    onImageComplete(data) {
      console.log('[useGeneration] SSE image_complete', data.images?.length, 'images')
  ...
  const prepare = useCallback((params: GenerationParams) => {
    ...
    setState({ ...initialState, status: 'preparing', totalBatch: params.batchCount, activeParams: params })
  }, [])
  ...
  const execute = useCallback(async () => {
    ...
    if (!params) {
      ...
      if (stored) {
        try { params = JSON.parse(stored) } catch { /* invalid */ }
      }
    }
    if (!params) return
    ...
    try {
      ...
      console.log('[execute] done, generationId =', generationId, 'promptIds =', promptIds)
