...
describe("getGenerations", () => {
	...
	it("filters by search query (case-insensitive, prompt match)", async () => {
		const a = makeGeneration({
			id: "a",
			params: { ...baseParams, prompt: "A Cat In Space" },
		});
		const b = makeGeneration({
			id: "b",
			params: { ...baseParams, prompt: "a dog in the park" },
		});
		mockRead([a, b]);
	...
	it("paginates results (page 1)", async () => {
		...
		const { items, total } = await getGenerations({
...
describe("createGeneration", () => {
	it('creates a generation with status "generating" and empty images', async () => {
		...
		const result = await createGeneration({
			data: { personaId: "persona-1", params: baseParams },
		});
	...
	it("assigns a unique id (UUID format)", async () => {
		...
		const result = await createGeneration({
			data: { personaId: "p", params: baseParams },
		});
	...
	it("persists the new generation to disk", async () => {
		...
		const written: Generation[] = JSON.parse(
...
describe("updateGeneration", () => {
	it("updates status of an existing generation", async () => {
		...
		const result = await updateGeneration({
			data: { id: "g1", updates: { status: "completed" } },
		});
	...
	it("throws when generation id is not found", async () => {
		...
		await expect(
			updateGeneration({
				data: { id: "missing", updates: { status: "completed" } },
			}),
