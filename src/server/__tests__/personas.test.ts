...
describe("createPersona", () => {
  ...
  it("persists the new persona to disk", async () => {
    ...
    const written: Persona[] = JSON.parse(
...
describe("updatePersona", () => {
  it("updates name of an existing persona", async () => {
    ...
    const result = await updatePersona({
      data: { id: "p1", updates: { name: "New Name" } },
    });
  ...
  it("updates loraStrength without touching other fields", async () => {
    ...
    const result = await updatePersona({
      data: { id: "p1", updates: { loraStrength: 0.9 } },
    });
...
describe("deletePersona", () => {
  ...
  it("throws when persona id is not found", async () => {
    ...
    await expect(deletePersona({ data: { id: "missing" } })).rejects.toThrow(
      "Persona not found: missing",
    );
  });
