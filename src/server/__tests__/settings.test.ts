...
describe('getSettings', () => {
  it('returns parsed settings from file', async () => {
    const stored = { serverUrl: 'http://localhost:9000', defaults: { cfg: 5, steps: 20, seedMode: 'fixed' } }
...
describe('updateSettings', () => {
  it('deep-merges partial update into existing settings', async () => {
    const current = { serverUrl: 'http://127.0.0.1:8188', defaults: { cfg: 7.5, steps: 30, seedMode: 'random' } }
    ...
    const result = await updateSettings({ data: { defaults: { steps: 50 } } as any })
  ...
  it('writes the merged settings to disk', async () => {
    vi.mocked(readFile).mockResolvedValueOnce(JSON.stringify(DEFAULT_SETTINGS) as any)
    vi.mocked(writeFile).mockResolvedValueOnce(undefined as any)

    await updateSettings({ data: { serverUrl: 'http://192.168.1.1:8188' } as any })
  ...
  it('calls resetComfyApi when serverUrl changes', async () => {
    ...
    await updateSettings({ data: { serverUrl: 'http://new-host:8188' } as any })
  ...
  it('does NOT call resetComfyApi when serverUrl is explicitly the same value', async () => {
    ...
    await updateSettings({ data: { serverUrl: DEFAULT_SETTINGS.serverUrl } as any })
  ...
  it('returns the fully merged settings object', async () => {
    ...
    const result = await updateSettings({ data: { defaults: { cfg: 4.0 } } as any })
...
describe('deepMerge behaviour (exercised via updateSettings)', () => {
  it('primitive from source overwrites target', async () => {
    const current = { serverUrl: 'http://old', defaults: { cfg: 7.5, steps: 30, seedMode: 'random' } }
    vi.mocked(readFile).mockResolvedValueOnce(JSON.stringify(current) as any)
    vi.mocked(writeFile).mockResolvedValueOnce(undefined as any)

    const result = await updateSettings({ data: { serverUrl: 'http://new' } as any })
