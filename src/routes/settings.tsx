...
function SettingsPage() {
  ...
  const [defaultSeedMode, setDefaultSeedMode] = useState<'random' | 'fixed'>('random')
      ...
      <div className="flex flex-col gap-5 px-6 pt-4 pb-5">
        ...
        <div className="rounded-2xl bg-white p-4 [box-shadow:0_2px_12px_#1A191808]">
          ...
          <div className="flex items-center justify-between">
            <span className="text-[14px] font-medium text-text">Server URL</span>
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-[6px]">
              <div className={`h-2 w-2 rounded-full ${isOnline ? 'bg-[#3D8A5A]' : 'bg-[#9C9B99]'}`} />
              <span className={`text-[13px] font-medium ${isOnline ? 'text-[#3D8A5A]' : 'text-[#9C9B99]'}`}>
        ...
        <div className="rounded-2xl bg-white p-4 [box-shadow:0_2px_12px_#1A191808]">
          {/* Default CFG */}
          <div className="flex items-center justify-between">
            <span className="text-[14px] font-medium text-text">Default CFG</span>
            {editingField === 'cfg' ? (
              <input
                type="number"
                value={cfg}
                ...
                onBlur={() => {
                  saveSettings({ defaults: { cfg, steps: defaultSteps, seedMode: defaultSeedMode } })
                  setEditingField(null)
                }}
                onKeyDown={(e) => {
                  if (e.key === 'Enter') {
          ...
          {/* Default Steps */}
          <div className="flex items-center justify-between">
            <span className="text-[14px] font-medium text-text">Default Steps</span>
            {editingField === 'steps' ? (
              <input
                type="number"
                value={defaultSteps}
                onChange={(e) => setDefaultSteps(parseInt(e.target.value, 10) || 1)}
                onBlur={() => {
                  saveSettings({ defaults: { cfg, steps: defaultSteps, seedMode: defaultSeedMode } })
                  setEditingField(null)
                }}
        ...
        <div className="rounded-2xl bg-white p-4 [box-shadow:0_2px_12px_#1A191808]">
          ...
          <div className="my-[14px] h-px bg-[#E5E4E1]" />
          <div className="flex items-center justify-between">
            <span className="text-[14px] font-medium text-text">ComfyUI Version</span>
